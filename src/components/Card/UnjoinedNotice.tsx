...
type Props =
	| { message: string; resource?: never }
	| { message?: never; resource: string };

export default function UnjoinedNotice({ message, resource }: Props) {
	return (
		<div className="flex flex-col gap-2 items-center px-10 py-6 place-self-center rounded-xl bg-slate-100 dark:bg-zinc-900 w-fit">
			<p className="p-1">
				{message ?? `Create or join a group to view ${resource}s`}
			</p>
			<Link
